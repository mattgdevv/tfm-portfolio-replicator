...
class PortfolioReplicator:
    ...
    async def test_variation_analysis(self):
        ...
        try:
            ...
            if iol_session:
                unified_analysis.set_iol_session(iol_session)
                print("🔴 Modo: COMPLETO (IOL + Finnhub/Yahoo)")
            else:
                print("🟡 Modo: LIMITADO (BYMA + Finnhub/Yahoo)")
    ...
    async def refresh_ccl_cache(self):
        ...
        try:
            ...
            for key in ["ccl:dolarapi_ccl", "ccl:ccl_al30", "ccl:ccl_implicit_yahoo"]:
    ...
    async def evaluate_alerts_now(self, file_path: str | None = None, threshold: float | None = None):
        """Ejecuta un análisis de arbitraje one-off a partir de un Excel/CSV seleccionado o el de .prefs.json."""
        print("\n🔔 Evaluar alertas ahora (Excel)")
        try:
            prefs = self._read_prefs()
            # Selección rápida de archivo
            if not file_path:
                default_file = prefs.get("SCHEDULER_FILE")
                prompt = f"📎 Archivo Excel/CSV para analizar (Enter para usar .prefs.json: {default_file}): "
                chosen = input(prompt).strip()
                file_path = chosen or default_file
            if not file_path:
            ...
            if not path.exists():
                ...
                return
            portfolio = await self.portfolio_processor.process_file(str(path), broker="AlertsExcel")
            # Silenciar logs del detector durante salida
            detector_logger = logging.getLogger("app.services.arbitrage_detector")
            prev_lvl = detector_logger.level
            detector_logger.setLevel(logging.ERROR)
            try:
                if threshold is None:
                    default_thr = self._read_prefs().get("ARBITRAGE_THRESHOLD", 0.005)
                    threshold_input = input(f"Umbral de arbitraje (Enter para {default_thr}): ").strip()
                    threshold = float(threshold_input) if threshold_input else float(default_thr)
                opportunities = await self.portfolio_processor.detect_arbitrage_opportunities(
                    portfolio, iol_session=None, threshold_percentage=threshold
                )
            finally:
                detector_logger.setLevel(prev_lvl)
            if opportunities:
                print(f"💰 {len(opportunities)} oportunidades detectadas (umbral={threshold*100:.2f}%)")
            else:
                print(f"✅ Sin oportunidades (umbral={threshold*100:.2f}%)")
        except Exception as e:
    ...
    async def scheduler_menu(self):
        while True:
            print("\n🗓️  Scheduler")
            print("1. Ver/Editar configuración (.prefs.json)")
            print("2. Iniciar scheduler (background)")
            print("3. Detener scheduler")
            print("4. Estado")
            print("5. Volver")
            op = input("Elige una opción (1-5): ").strip()
            if op == '1':
                self._scheduler_edit_config()
            elif op == '2':
                self._scheduler_start_background()
            elif op == '3':
                self._scheduler_stop_background()
            elif op == '4':
    ...
    def _scheduler_view_config(self):
        prefs = self._read_prefs()
        if not prefs:
            print("(sin .prefs.json)")
            return
        print(json.dumps(prefs, indent=2, ensure_ascii=False))

    def _scheduler_edit_config(self):
        prefs = self._read_prefs()
        # Defaults
        source = prefs.get('SCHEDULER_SOURCE', 'excel')
        file_path = prefs.get('SCHEDULER_FILE', '')
        interval = prefs.get('SCHEDULER_INTERVAL_MINUTES', 60)
        tz = prefs.get('SCHEDULER_TIMEZONE', 'UTC')
        jitter = prefs.get('SCHEDULER_JITTER_SECONDS', 0)
        alerts_enabled = bool(prefs.get('ALERTS_ENABLED', False))
        alerts_interval = prefs.get('ALERTS_INTERVAL_MINUTES', 30)
        threshold = prefs.get('ARBITRAGE_THRESHOLD', 0.005)

        print("\n🔧 Editar configuración (.prefs.json) — Enter para mantener")
        n_source = input(f"SCHEDULER_SOURCE [excel/iol] ({source}): ").strip() or source
        n_file = input(f"SCHEDULER_FILE (ruta) ({file_path}): ").strip() or file_path
        try:
            n_interval = input(f"SCHEDULER_INTERVAL_MINUTES ({interval}): ").strip()
            n_interval = int(n_interval) if n_interval else interval
        except ValueError:
            print("⚠️  Valor inválido, manteniendo")
            n_interval = interval
        n_tz = input(f"SCHEDULER_TIMEZONE ({tz}): ").strip() or tz
        try:
            n_jitter = input(f"SCHEDULER_JITTER_SECONDS ({jitter}): ").strip()
            n_jitter = int(n_jitter) if n_jitter else jitter
        except ValueError:
            print("⚠️  Valor inválido, manteniendo")
            n_jitter = jitter
        a_enabled_in = input(f"ALERTS_ENABLED [s/n] ({'s' if alerts_enabled else 'n'}): ").strip().lower()
        n_alerts_enabled = alerts_enabled if a_enabled_in not in ('s','n') else (a_enabled_in == 's')
        try:
            n_alerts_interval = input(f"ALERTS_INTERVAL_MINUTES ({alerts_interval}): ").strip()
            n_alerts_interval = int(n_alerts_interval) if n_alerts_interval else alerts_interval
        except ValueError:
            print("⚠️  Valor inválido, manteniendo")
            n_alerts_interval = alerts_interval
        try:
            n_threshold = input(f"ARBITRAGE_THRESHOLD ({threshold}): ").strip()
            n_threshold = float(n_threshold) if n_threshold else threshold
        except ValueError:
            print("⚠️  Valor inválido, manteniendo")
            n_threshold = threshold

        prefs.update({
            'SCHEDULER_SOURCE': n_source,
            'SCHEDULER_FILE': n_file,
            'SCHEDULER_INTERVAL_MINUTES': n_interval,
            'SCHEDULER_TIMEZONE': n_tz,
            'SCHEDULER_JITTER_SECONDS': n_jitter,
            'ALERTS_ENABLED': n_alerts_enabled,
            'ALERTS_INTERVAL_MINUTES': n_alerts_interval,
            'ARBITRAGE_THRESHOLD': n_threshold,
        })
        self._write_prefs(prefs)
        print("✅ Configuración guardada en .prefs.json")

    def _scheduler_pid_path(self) -> Path:
        return Path('.scheduler.pid')
    ...
    def _scheduler_start_background(self):
        pid_file = self._scheduler_pid_path()
        if pid_file.exists():
            try:
                pid = int(pid_file.read_text().strip())
                if self._process_alive(pid):
                    print(f"⚠️  Scheduler ya está corriendo (PID {pid})")
                    return
            except Exception:
                pass
            # limpiar pid obsoleto
        ...
        try:
            with open(log_path, 'a', encoding='utf-8') as lf:
                # Lanzar scheduler como proceso background
                proc = subprocess.Popen([
                    sys.executable, 'scripts/scheduler.py'
                ], stdout=lf, stderr=lf, env=env)
                pid_file.write_text(str(proc.pid), encoding='utf-8')
            print(f"✅ Scheduler iniciado (PID {proc.pid}). Logs: {log_path}")
        except Exception as e:
            print(f"❌ No se pudo iniciar el scheduler: {e}")

    def _scheduler_stop_background(self):
        pid_file = self._scheduler_pid_path()
        if not pid_file.exists():
            print("ℹ️  Scheduler no está corriendo (no hay .scheduler.pid)")
            return
        try:
            pid = int(pid_file.read_text().strip())
        except Exception:
            print("⚠️  .scheduler.pid inválido; eliminando archivo")
            pid_file.unlink(missing_ok=True)
            return
        try:
            ...
            # esperar breve
            time.sleep(0.5)
            print(f"✅ Señal enviada al PID {pid}")
        except Exception as e:
            print(f"⚠️  No se pudo enviar señal: {e}")
        finally:
            pid_file.unlink(missing_ok=True)
            print("🧹 .scheduler.pid eliminado")

        # Aviso si sigue vivo
        if self._process_alive(pid):
            print("⚠️  El proceso aún parece vivo; puede terminar por sí solo en breve.")
    ...
    def _scheduler_status(self):
        ...
        if pid_file.exists():
            try:
                pid = int(pid_file.read_text().strip())
                running = self._process_alive(pid)
            except Exception:
                running = False

        print(f"Estado: {'RUNNING' if running else 'STOPPED'} (PID {pid if pid else '-'})")
        # Mostrar status del último job si existe
        try:
            status_path = Path('output') / 'scheduler_status.json'
            if status_path.exists():
                import json as _json
                status = _json.loads(status_path.read_text(encoding='utf-8'))
                last = status.get('last_run_at')
                ok = status.get('last_run_ok')
                err = status.get('last_error')
                print(f"Último job: {'OK' if ok else 'ERROR'} @ {last if last else '-'}")
                if not ok and err:
                    print(f"Último error: {err}")
            else:
                print("Sin status previo (aún no hay corridas)")
        except Exception as e:
            print(f"⚠️  No se pudo leer status: {e}")
